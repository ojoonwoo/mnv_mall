      ...
      <div class="contents l2 clearfix">
        <div class="section main">
          ...
          <div class="area_main_middle nopadd noborder">
            <div class="table_block wish borderT" id="qna_board_area">

              <form name="frm_execute" method="POST">
                <input type="hidden" name="pg" value="<?=$pg?>">
              </form>
<?
	$buyer_count_query = "SELECT count(*) FROM ".$_gl['board_qna_table']."";
...
<?
	 ...
	 $buyer_list_query = "SELECT board.idx, board.goods_code, board.subject, board.date, board.user_id, goods.goods_img_url FROM ".$_gl['board_qna_table']." AS board, ".$_gl['goods_info_table']." AS goods WHERE 1 AND board.goods_code = goods.goods_code AND board.depth=0 ORDER BY board.thread DESC LIMIT $PAGE_CLASS->page_start, $page_size";
...
?>
                  <tr>
                    ...
                    <td class="subject">
                      <a href="<?=$_mnv_PC_board_url?>read_qna.php?idx=<?=$buyer_info[$key]['idx']?>&goods_code=<?=$buyer_info[$key]['goods_code']?>&pg=<?=$pg?>">
