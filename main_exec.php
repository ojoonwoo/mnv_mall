...
// 회원수정시 회원본인인지 체크 --------------------------> 현재 안쓰는 코드
		...
		case "insert_order_info" :
			...
			if ($order_result)
			{
				...
				$innerHTML .= "document.getElementById('LGD_PAYINFO').action = '".$_SERVER['DOCUMENT_ROOT']."/lib/LGU+_XPay_Crossplatform_PHP/payres.php';";
				$innerHTML .= "document.getElementById('LGD_PAYINFO').submit();";
				$innerHTML .= "} else {";
				$innerHTML .= "closeIframe();";
				$innerHTML .= "}}";
				$innerHTML .= "</script>";
				$innerHTML .= "<img src='".$_mnv_PC_images_url."logo.png'>";
