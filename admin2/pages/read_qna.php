<?
	...
	$hit_query = "UPDATE ".$_gl['board_qna_table']." SET hit=hit+1 WHERE idx='".$idx."'";
	$result = mysqli_query($my_db, $hit_query); 

	$read_query = "SELECT * FROM ".$_gl['board_qna_table']." WHERE idx='".$idx."'";
...
?>
  ...
  <div id="page-wrapper">
    <div class="row">
      <div class="col-lg-12">
        <h1 class="page-header">Q&A 읽기</h1>
      </div>
      <!-- /.col-lg-12 -->
    </div>
    <!-- /.row -->
  ...
  function go_page(action) {
    switch (action) {
      case "list":
        ...
        break;
      case "reply":
        location.href="reply_qna.php?idx=<?=$idx?>";
        break;
      case "edit":
        location.href="edit_qna.php?idx=<?=$idx?>";
        break;
      case "delete":
        location.href="delete_qna.php?idx=<?=$idx?>";
        break;
    }
  }
